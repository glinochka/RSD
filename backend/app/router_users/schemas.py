...
class User_from_tg(BaseModel):
    username: str = Field(..., min_length=3, max_length=32, description="Имя пользователя: длина от 3 до 32 символов")
...
class Update_userSubscription(BaseModel):
    ...
    subscription_type: Optional[Literal['Free', 'Advanced', 'Pro']] = Field(description="Тип подписки ('Free', 'Advanced', 'Pro')")
    subscription_end_date: Optional[date] = Field(description="Дата окончания подписки")
