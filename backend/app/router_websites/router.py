...
async def _run_website_generation(
    ...
    async with async_session_maker() as session:
        async with session.begin():
            ...
            website = await website_dao.find_one_by_filter(id=website_id)
            if not website:
                logger.error(f"Website not found for generation: {website_id}")
                return

            await website_dao.set_generation_status(website, "generating")

    # Gather agent data if linked
    services = []
    contacts = {}
    if request.agent_id:
        try:
            ...
            logger.warning(f"Failed to fetch agent services: {e}")

        try:
            contacts = await _get_agent_contacts(request.agent_id)
        except Exception as e:
            logger.warning(f"Failed to fetch agent contacts: {e}")
    ...
    try:
        ...
        try:
            ...
            logger.exception(f"Failed to update generation status for {website_id}")
...
async def _run_website_export(
    ...
    try:
        ...
        if result.success:
            ...
            _export_jobs[export_id]["error_message"] = result.error_message or "Unknown export error"
            logger.error(f"Export failed: {export_id}, error: {result.error_message}")
