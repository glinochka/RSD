...
_STATE_PROMPTS: dict[DialogState, str] = {
    DialogState.GREET: "Коротко поприветствуй и спроси, чем помочь (одно предложение).",
    DialogState.LISTEN: "Выслушай запрос и ответь по сути; не задавай больше одного уточняющего вопроса.",
    DialogState.CLARIFY: "Задай ровно один короткий уточняющий вопрос, без цепочки вопросов.",
    DialogState.ACT: "Выполни действие (запись, CRM) или объясни следующий шаг.",
    DialogState.CONFIRM: "Переспроси критичные данные (дата, время, телефон) одной фразой.",
    DialogState.CLOSE: "Вежливо завершай: «Могу ещё чем-то помочь?» — если отказ, попрощайся.",
    DialogState.HANDOFF: "Скажи, что соединяешь с оператором, и не затягивай.",
}
...
def decide_orchestrator(
    ...
    *,
    ...
    if barged_in:
        prompt_addon = (
            "Абонент перебил твою реплику. Начни с «Слышал, вы…» и ответь на новую мысль."
            + (f" Прерванная фраза: «{interrupted_agent_text[:200]}»." if interrupted_agent_text else "")
        )
